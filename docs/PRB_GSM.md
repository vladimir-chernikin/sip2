...
```python
# В audiosocket/main.py после создания ExternalMedia
async def send_first_rtp_packet(remote_addr: tuple[str, int]):
    """Отправляет пустой RTP пакет для инициализации соединения"""
    header = struct.pack("!BBHII", 0x80, 8, 0, 0, 0x12345678)  # PT=8 (PCMA)
    transport.sendto(header, remote_addr)
```
...
**Минусы:**
- Нужно синхронизировать создание ExternalMedia в backend
- Пакет будет "пустым" (нет полезной нагрузки)
